...
```bash
cd D:\TotsFiles
uv run --no-sync --no-build python Coral_Key_in_Three_Hour_Epochs/baselines/run_coral_key_baselines.py --smoke-test
uv run --no-sync --no-build python Coral_Key_in_Three_Hour_Epochs/baselines/run_coral_key_baselines.py --workers 8
```
